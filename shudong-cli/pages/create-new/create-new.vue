				...
				<view style="width: 100%;">
					<view class="colorWrapper">
						<block v-for="(option,index) in borderColorInfo" :key="index">
							<view :class="[{color:true},{current:showWhite[index]}]" :style="{border:option}" @tap="getCurrentColor(index)"></view>
						</block>
					</view>
				</view>
			</view>
			...
			<view class="colorSelection">
				...
				<view class="colorWrapper">
					<block v-for="(option,index) in staffIconInfo" :key="index">
						<image :src="option" mode="aspectFit" :class="[{color2:true},{current:showWhite2[index]}]" @tap="getCurrentColor2(index)"></image>
					</block>
				</view>
			</view>
			<!-- 确认按钮 -->
